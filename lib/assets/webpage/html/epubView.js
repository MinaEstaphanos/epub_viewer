...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss) {
  ...
  rendition.hooks.content.register((contents) => {
    ...
    // The selected text when the touch began (only while a selection is
    // active). Used to tell a selection-handle drag from a page-turn swipe:
    // dragging a handle changes the selection, a swipe never does.
    var touchStartSelection = null;
    ...
    var touchStartHandler = function (e) {
      ...
      if (hasActiveSelection()) {
        ...
        if (e.touches && e.touches.length > 0) {
          ...
          touchStartSelection = currentSelectionText();
        }
        ...
        if (e.touches && e.touches.length > 0 && touchStartX !== null) {
          ...
          if (deltaX > 10 || deltaY > 10) {
            ...
            if ((deltaX > 30 && deltaX > deltaY * 1.5) || deltaX > 50) {
              // Not a swipe if the selection is changing under the finger: on
              // WKWebView, dragging a selection handle delivers these touchmove
              // events to the page (Android's native selection layer keeps
              // them), and clearing here killed the selection after a couple
              // of words. Let the handle drag through untouched.
              if (touchStartSelection !== null &&
                  currentSelectionText() !== touchStartSelection) {
                return;
              }
              clearSelection();
              e.preventDefault();
