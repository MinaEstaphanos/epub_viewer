...
// Selection-handle drag detection (see the touch handlers). WKWebView delivers
// a handle drag to the page as ordinary touchmove events, which the
// swipe-while-selected guard would otherwise treat as a page-turn swipe.
var touchInProgress = false;
var selectionChangedDuringTouch = false;
var pageProgression = 'ltr'; // 'rtl' for right-to-left books (Arabic): flips the swipe-to-page mapping
...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss) {
  ...
  // The viewer height is governed by CSS (html/body/#viewer are height:100%,
  // see swipe.html). An earlier line here set #viewer.style.height to
  // window.innerHeight with no unit, which is invalid and did nothing; making
  // it a fixed px value would re-introduce the stale-height gap after the
  // WebView resizes, so it is intentionally left to CSS.
  ...
  rendition = book.renderTo("viewer", {
    ...
    // The layout always stays ltr. `direction` describes the book's page
    // progression (rtl for Arabic) and is used only to map swipe gestures,
    // see pageProgression below. Handing 'rtl' to epub.js here switches the
    // continuous manager into its rtl layout, which rendered blank pages and
    // ignored swipes on WKWebView (tested 2026-09-10).
    defaultDirection: 'ltr'
  });
  pageProgression = direction === 'rtl' ? 'rtl' : 'ltr';

  // Apply initial theme
  updateTheme(backgroundColor, foregroundColor, customCss);
  // iOS/WebKit: keep programmatic scrolls from being reverted by the engine
  // (see installScrollEchoGuard below). Installed once the manager's
  // container exists.
  rendition.on('attached', function () { installScrollEchoGuard(); });
  ...
  rendition.hooks.content.register(function (contents) {
    ...
    contents.window.document.addEventListener('selectionchange', function () {
      ...
      // A selection that changes while a finger is down is a handle drag.
      if (touchInProgress && hasNonCollapsedRange) {
  ...
  rendition.hooks.content.register((contents) => {
    ...
    // Telling a selection-handle drag from a page-turn swipe. Three signals,
    // any one of which marks the gesture as a handle drag: the selection
    // changed while the finger was down (selectionChangedDuringTouch), the
    // selected text differs from what the previous touch event saw
    // (lastSeenSelection), or the touch began on a handle (touchOnHandle).
    // A swipe triggers none of them. The text comparison is not enough on its
    // own: WKWebView updates the DOM selection lazily during a drag.
    var lastSeenSelection = null;
    ...
    function touchNearSelectionHandle(x, y) {
      try {
        var sel = contents.window.getSelection();
        ...
        var r = 48; // touch-target radius around each handle
        var nearStart = Math.abs(x - first.left) <= r && y >= first.top - r && y <= first.bottom + r;
        var nearEnd = Math.abs(x - last.right) <= r && y >= last.top - r && y <= last.bottom + r;
    ...
    function isHandleDrag() {
      if (touchOnHandle || selectionChangedDuringTouch) return true;
      var now = currentSelectionText();
      var changed = lastSeenSelection !== null && now !== lastSeenSelection;
    ...
    var touchStartHandler = function (e) {
      ...
      if (hasActiveSelection()) {
        ...
        if (e.touches && e.touches.length > 0) {
          ...
          touchOnHandle = touchNearSelectionHandle(touchStartX, touchStartY);
        }
      ...
      lastSeenSelection = currentSelectionText();
    };
      ...
      if (hasActiveSelection()) {
        ...
        if (e.touches && e.touches.length > 0 && touchStartX !== null) {
          ...
          if (deltaX > 10 || deltaY > 10) {
            hasMoved = true;
            // A horizontal page-turn swipe while text is selected. Swipes are
            // blocked while a selection is active, and on Android a tap does not
            // reach the page to clear the selection (the native selection layer
            // swallows it), which left the reader frozen with no way out. Clear
            // the selection here instead: this touchmove is the very handler that
            // blocks the swipe, so it reliably fires on the user's swipe. The
            // block still consumes this gesture; the next swipe pages normally.
            if ((deltaX > 30 && deltaX > deltaY * 1.5) || deltaX > 50) {
              // Not a swipe if this is a selection-handle drag: WKWebView
              // delivers handle drags to the page as touchmove events (Android's
              // native selection layer keeps them), and clearing here killed the
              // selection after a couple of words. Let the drag through.
              if (isHandleDrag()) {
                return;
              }
              clearSelection();
      ...
      if (hasActiveSelection()) {
        ...
        if (touchStartX !== null && currentTouchX !== null && touchStartTime !== null) {
          ...
          if (!hasMoved && deltaX < 10 && deltaY < 10 && touchDuration < 300) {
            // A tap while a selection is active: clear it. The browser's default
            // tap-to-deselect does not reset the isSelecting/lastCfiRange tracking
            // flags, so hasActiveSelection() would stay true and keep the
            // swipe/scroll gesture block on with no way for the reader to release
            // it. clearSelection() drops the selection, resets those flags and
            // notifies the host so the block is lifted.
            clearSelection();
          } else if (isHandleDrag()) {
            // A selection-handle drag ending: let the browser finish it.
    ...
    // Only page-turn on horizontal swipes in paginated flow. In scrolled
    // (continuous) mode a sideways drift while scrolling would otherwise jump
    // a page and skip content.
    if (useCustomSwipe && flow !== 'scrolled') {
      ...
      if (el) {
        detectSwipe(el, function (el, direction) {
          ...
          // Map the gesture to the page progression: in an ltr book the next
          // page is to the right (swipe left to reach it); in an rtl book
          // (e.g. Arabic) the next page is to the left, so a left-to-right
          // swipe advances. rendition.next()/prev() are already direction-
          // aware; only the gesture mapping has to flip.
          var rtl = pageProgression === 'rtl' ||
            (rendition && rendition.settings && rendition.settings.direction === 'rtl');
          var forwardSwipe = rtl ? 'r' : 'l';
          var backwardSwipe = rtl ? 'l' : 'r';
          if (direction == forwardSwipe) {
            // Double-check before calling
            if (!hasActiveSelection()) {
              rendition.next();
            }
          }
          if (direction == backwardSwipe) {
            // Double-check before calling
            if (!hasActiveSelection()) {
              rendition.prev();
            }
  ...
  // This fork treats the font size as a percentage (see setFontSizePercentage),
  // and the app passes a percentage (e.g. 100). Apply it as "%" on the first
  // paint so the text opens at the right size. Applying it as "px" made 100
  // render as 100px — a huge size that looked like ~200% — until the app's
  // restore step corrected it a moment later.
  rendition.themes.fontSize(fontSize + "%");
  // Re-apply the theme with the custom CSS too. This call used to omit
  // customCss: themes.register() replaces the stored rules outright, and
  // sections only pick up the rules current when they render (after
  // loadBook returns), so every EpubTheme.customCss rule was silently
  // dropped on the initial load.
  updateTheme(backgroundColor, foregroundColor, customCss);
}
...
function setFontSizePercentage(fontSize) {
  rendition.themes.fontSize(`${fontSize}%`);
  ...
  ele.addEventListener('touchend', function (e) {
    ...
    if (hasActiveSelection()) {
      if (swipe_det.touchStartTime !== null) {
        ...
        // If minimal movement and short duration, it's a tap - clear the
        // selection so the gesture block is released (see the touchend handler
        // above for why the browser's own deselect is not enough).
...
// ===== Scroll echo guard (WebKit) =====
// On iOS WKWebView the engine's asynchronous scrolling can echo an *older*
// programmatic scroll position back to the page shortly after a newer one was
// set, silently reverting the newer scroll. If the content shrank in between
// (e.g. the views were cleared for a new section) the echoed value arrives
// clamped to the container's current scroll range. epub.js's continuous view
// manager compensates for prepended sections with relative scrollBy() calls,
// so one lost scroll leaves the viewport a whole section (or many pages) off:
// the "TOC jump lands on the previous chapter" bug.
//
// This guard remembers the position each programmatic scroll asked for. When
// a scroll event shortly after a programmatic scroll reports one of the
// superseded programmatic positions (clamped to the current range) instead of
// the intended one, that is an echo and the intended position is re-applied.
// Any other position is a genuine (user/browser) scroll and is adopted as-is,
// so the guard never fights the reader's finger.
function installScrollEchoGuard() {
  var m = rendition.manager;
  var c = m && m.container;
  if (!c || m.__echoGuard) return;
  m.__echoGuard = true;

  var ECHO_WINDOW_MS = 500; // echoes were observed 20-150 ms after the scroll
  var MAX_STALE = 6;
  var intended = null;      // {l, t}: the latest programmatic target
  var intendedAt = 0;
  var stale = [];           // superseded programmatic targets, newest last

  function now() { return performance.now(); }
  function near(a, b) { return Math.abs(a - b) <= 1; }
  function current() { return { l: c.scrollLeft, t: c.scrollTop }; }
  function clampToRange(pos) {
    // Horizontal range is [0, max] for ltr and [-max, 0] for rtl
    // (modern engines report rtl scrollLeft as negative).
    var maxL = Math.max(0, c.scrollWidth - c.clientWidth);
    var maxT = Math.max(0, c.scrollHeight - c.clientHeight);
    var l = pos.l, t = pos.t;
    if (m.settings.direction === 'rtl') {
      l = Math.min(0, Math.max(-maxL, l));
    } else {
  ...
  function samePos(a, b) { return near(a.l, b.l) && near(a.t, b.t); }
  function noteProgrammatic() {
    var pos = current();
    if (intended && !samePos(intended, pos)) {
      stale.push(intended);
      if (stale.length > MAX_STALE) stale.shift();
    }
    intended = pos;
    intendedAt = now();
  }
  function reassert(why) {
    if (!intended) return;
    var pos = current();
    if (samePos(pos, intended)) return;
    var withinWindow = (now() - intendedAt) <= ECHO_WINDOW_MS;
    var isEcho = withinWindow && stale.some(function (s) { return samePos(clampToRange(s), pos); });
    if (!isEcho) {
      // A real scroll: adopt it and stop guarding until the next programmatic move.
      intended = null;
      stale = [];
      return;
    }
    console.log('SCROLL-ECHO-GUARD ' + why + ': browser reported ' + pos.l + ',' + pos.t +
      ' (echo of a superseded scroll) - re-applying ' + intended.l + ',' + intended.t);
    c.scrollLeft = intended.l;
    c.scrollTop = intended.t;
    // Keep the manager's cached offsets in step so its next check() uses them.
    m.scrollLeft = c.scrollLeft;
    m.scrollTop = c.scrollTop;
    intendedAt = now();
  }
  function scheduleChecks() {
    setTimeout(function () { reassert('timer60'); }, 60);
    setTimeout(function () { reassert('timer200'); }, 200);
  }

  var origScrollTo = m.scrollTo.bind(m);
  m.scrollTo = function (x, y, silent) {
    origScrollTo(x, y, silent);
    noteProgrammatic();
    scheduleChecks();
  };
  var origScrollBy = m.scrollBy.bind(m);
  m.scrollBy = function (x, y, silent) {
    origScrollBy(x, y, silent);
    noteProgrammatic();
    scheduleChecks();
  };
  c.addEventListener('scroll', function () { reassert('scroll'); });
}
